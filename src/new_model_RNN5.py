...
def m_kl(y_true, y_pred):
    ...
    loss = K.mean(K.sum(true_y * K.log(true_y / y_pred), axis=-1))
    return  loss
    # condition = K.greater(K.sum(y_true), 0)
    # return K.switch(condition, loss, K.zeros_like(loss))
...
def G2V(file_name, num_walks, walk_length, _dataset, X, cv_splits, features, y,l_dim,n_clusters, is_supervised=True):
    ...
    for i in range(fold):
        ...
        with sess.as_default():
            ...
            Y = Input(shape=(n_labels,), dtype='float32')
            ...
            # Z_X = Dense(n_clusters, activation='softmax', name='s1')(output_dense)
            # Z_Y = Dense(n_clusters, activation='softmax')(output_dense)
            # x_ohe = Lambda(K.one_hot,
            #                arguments={'num_classes': n_clusters},
            #                output_shape=(K.shape(output_dense)[0], n_clusters))(K.argmax(Z_X, -1))
            ...
            if is_supervised == False:
                ...
                output = Dense(y.shape[1], activation='sigmoid', name='su')(output)
                output =  Concatenate(axis=-1)([output, clustering_layer])
                model = Model(inputs=[document,Y], outputs=output)
            else:
                # output = Concatenate(axis=-1)([output_dense, x_ohe])
                output = Dense(y.shape[1], activation='sigmoid', name='su')(output_dense)
                model = Model(inputs=[document,Y], outputs=output)

            class DataSequence(Sequence):

                def __init__(self, x_l, x_ul, y_l, y_ul, batch_size, _type="train"):
                    self._type = _type
                    if self._type == "train":
                        if is_supervised == False:
                            self.xl, self.xu, self.y, self.y_ul, self.pl, self.pu = x_l, x_ul, y_l, y_ul, target_distribution(
                ...
                def __getitem__(self, idx):
                    if self._type == "train":
                        if idx < self.steps_l:
                            ...
                            batch_y = self.y_ul[(idx - self.steps_l) * self.batch_size:((idx - self.steps_l) + 1) * self.batch_size]#-1 * np.ones((batch_x.shape[0], self.y.shape[1]))
                            if is_supervised == False:
                                batch_y = np.hstack((batch_y,self.pu[
                                      (idx - self.steps_l) * self.batch_size:((idx - self.steps_l) + 1) * self.batch_size]))
                        batch_x = [batch_x, batch_y]
                        ...
                        batch_x = [batch_x, -1 * np.ones((batch_x.shape[0], self.y.shape[1]))]
            ...
            data_sequence_train = DataSequence(X_train, X_ul, y_train, y_ul, 128, "train")
            data_sequence_val = DataSequence(X_v, None,  y_v, None, 128, "val")
            optimizer = keras.optimizers.Adam(lr=0.001)

            def m_loss(y_true, y_pred):
                alpha = 0.99
                if is_supervised:
                    sup_loss = m_crossentropy(y_true, y_pred)# + m_kl(Z_Y, Z_X)
                    return sup_loss
                else:
                    sup_loss = m_crossentropy(y_true[:, :-n_clusters], y_pred[:, :-n_clusters])
            ...
            if is_supervised == False:
                model2 = Model(inputs=[document,Y], outputs=[output_dense])
            ...
            to_print = False
            data_sequence_test = DataSequence(X_test, None, y_test, None, 128, "test")
            if is_supervised == False and to_print == True:
                pred = model.predict_generator(data_sequence_test)
                ...
                predY =  model.predict_generator(data_sequence_test)[:,:-n_clusters]
            else:
                predY = model.predict_generator(data_sequence_test)
        # encode = Model(inputs=[document,Y], outputs=Z_X)
        # a = encode.predict_generator(data_sequence_test)
        # from scipy.sparse import csr_matrix
        # b = csr_matrix((np.ones(a.shape[0]), (y_test.argmax(-1), a.argmax(-1)))).toarray()
        # print(b.sum(0))
        acc += accuracy_score(y_test, np.round(predY))
        print(f1_score(y_test, np.round(predY), average='macro'))
