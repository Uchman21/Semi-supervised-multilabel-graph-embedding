...
def G2V( file_name, num_walks, walk_length, _dataset,train_mask,val_mask,u_mask,test_mask,l_dim,n_clusters,X, y,features, is_supervised=True):
    ...
    with sess.as_default():
        ...
        def vae_loss(y_true, y_pred):
            alpha = 0.99
            sup_loss = my_categorical_crossentropy(y_true,y_pred)
            if is_supervised:
                return sup_loss
            else:
        ...
        model.compile(loss=vae_loss, optimizer=optimizer, metrics=[my_categorical_accuracy])
